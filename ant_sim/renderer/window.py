...
APPLE = Apple(x=100, y=700, batch=MAIN_BATCH)
ANTHILL = Anthill(apple_pos=[APPLE.x, APPLE.y], x=700, y=100, batch=MAIN_BATCH)
...
def update(dt):
    ...
    for ant in ANTHILL.get_ants():
        ...
        if ant.should_drop_pheromone():
            PherManager.add_pheromone(ant, lifetime=5)
